...
class CalculateCommentAchievementAction
{
    public function handle(User $user)
    {
        ...
        Achievement::where('type','comment')
            ->each(function (Achievement $achievement) use ($user, $userComments){


                if($userComments->count() === $achievement->requirement) {
                    $commentIdsThatLedToAchievement = $userComments->pluck('id');
                    $achievementRecord = [
                        'name' => $achievement->name,
                        'user_id' => $user->id,
                        'achievement_id' => $achievement->id,
                        'metadata' => $commentIdsThatLedToAchievement,
                        'type' => 'comment',
                    ];
